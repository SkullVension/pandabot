...
export default {
    name: 'snowflake',
    description: 'Decodes a Discord Snowflake ID to extract its creation metadata.',
    aliases: ['sf'],
    category: 'misc',
    ...
    async execute(message, args) {
        ...
        if (!idStr) {
            return message.reply('❌ Please provide a valid Discord ID to decode. Example: `p!snowflake 123456789012345678`');
        }

        if (!/^\d+$/.test(idStr)) {
            return message.reply('❌ That doesn\'t look like a valid Discord ID. It must contain numbers only.');
        }
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '⏰ Relative Time', value: `<t:${unixTimestamp}:R>`, inline: true },
                    { name: '⚙️ Unix Milliseconds', value: `\`${timestampMs}\``, inline: true },
            ...
            return message.reply('❌ An error occurred while decoding that ID. Make sure it\'s a valid 64-bit integer.');
        }
    }
};
